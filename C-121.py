...
while (cap.isOpened()):
    ...
    mask_1 = cv2.morphologyX(mask_1, cv2.MORPH_OPEN, np.ones((3,3), np.uint8));
    mask_1 = cv2.morphologyX(mask_1, cv2.MORPH_DILATE, np.ones((3,3), np.uint8));
    ...
    val1 = cv2.bitwise_end(img, img, mask = mask_2);

    #Mantendo apenas a parte das imagens com a cor vermelha
    val2 = cv2.bitwise_end(bg, bg, mask = mask_1);

    #Gerando o resultado final mesclando res_1 e res_2
    final_output = cv2.addWeighted(val1, 1, val2, 1, 0);
